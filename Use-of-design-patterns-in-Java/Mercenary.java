...
public class Mercenary extends Enemy implements Interactable {
    ...
    private MovementStrategy moveStrategy = new MercenaryMovementStrategy();
    ...
    private boolean canBeBribed(Player player) {
        int distance = getManhattanDistance(player.getPosition(), this.getPosition());
    ...
    public void interact(Player player, Game game) {
        ...
        // if player has sceptre in inventory
        if (player.hasSceptre()) {
            Sceptre sceptre = player.getSceptre();

            // use sceptre on this merc
            sceptre.use(this, game);

            // set sceptre the player is using
            player.setSceptreInUse(sceptre);
        } else {
            allied = true;
            bribe(player);
        }
